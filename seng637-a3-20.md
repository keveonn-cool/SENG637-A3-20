...
We were unable to achieve proper statement coverage due to unreachable code in calculateColumnTotal and calculateRowTotal methods in the DataUtilities class. You can see this under the calculateColumnTotal and calculateRowTotal headings as well as the justification. We were able to achieve the minimum threshold for branching and method converges.
...
As you can see above, the red code is unreachable and therefore we cannot increase the statement coverage. We only covered 72.9% of statements. On the other end, we were able to achieve 50% branch coverage which is enough for to reach the minimum threshold. We also achieved 100% for method coverage. We did this by adding new tests that cover a more wide range of the conditions within the calculateColumnTotal method. 
