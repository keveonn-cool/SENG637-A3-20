...
We were unable to achieve proper statement coverage due to unreachable code in calculateColumnTotal and calculateRowTotal methods in the DataUtilities class. You can see this under the calculateColumnTotal and calculateRowTotal headings as well as the justification. We were able to achieve the minimum threshold for branching and method coverges.
...
calculateColumnTotal(Values2D data, int column), is a method that calculates the column total value in a column. As you can see above, the red code is unreachable and therefore we cannot increase the statement coverage. We only covered 72.9% of statements. We were able to achieve 50% branch coverage which is enough for to reach the minimum threshold, if averaged. Again, 50% branch coverege is because of the unreachable code. We also achieved 100% for method coverage. We did this by adding new tests that cover a more wide range of the conditions within the calculateColumnTotal method. We were able to achieve moderate coverage by using tests like this:
...
As you can see above, the red code is unreachable and therefore we cannot increase the statement coverage. We only covered 72.9% of statements. On the other end, we were able to achieve 50% branch coverage which is enough to reach the minimum threshold. We also achieved 100% for method coverage. We did this by adding new tests that cover a more wide range of the conditions within the calculateRowTotal method.
...
![Alt text](Images/range_statement.jpg)

Note: Please be aware that the displayed screen capture indicates a statement test coverage of 84% for the Range class. However, it should be noted that the remaining coverage was unattainable because of inaccessible code within the range class methods. Despite attempts to address this issue, additional tests were created following an analysis of the coverage through the EclEmma tool.
