...
public class RangeTest {
	...
	public void shiftByNegativeDoubleToGetNegativeValueUpperBound() {
		...
		assertEquals("The value for the upper bound is incorrect.", -0.8, testShiftedRange.getUpperBound(),.000000001d);
	}
    ...
    // Test for shift method with no zero crossing
    @Test
    public void testShiftWithNoZeroCrossing() {
        ...
        // Check if the lower bound shifted correctly
        assertEquals("The combined lower bound should be -1.0 and it is " + lowerBound,
                     -1.0, lowerBound, 0.000000001d);
        
        // Check if the upper bound shifted correctly
        assertEquals("The combined upper bound should be 4.0 and it is " + upperBound,
                     4.0, upperBound, 0.000000001d);
    }
	...
	// Verifies that the combine method correctly merges two valid ranges.
	@Test
	public void testCombineWithValidRanges() {
	    Range firstRange = new Range(-3, 0);
	    Range secondRange = new Range(0, 3);
	    Range combinedRange = Range.combine(firstRange, secondRange);

	    assertEquals("The lower bound should be", -5, combinedRange.getLowerBound(), .000000001d);
	    assertEquals("The upper bound should be", 5, combinedRange.getUpperBound(), .000000001d);
	}
    ...
    public void centralValuewithLargeBounds() {
	 ...
	 // This test verifies the functionality of findMinimum() and findMaximum() through the combineIgnoringNaN() method.
 ...
 public void testCombineIgnoringNaNWithMinAndMax() {
    ...
    @Test
    public void testExpandWithIntegers() {
        Range inputRange = new Range(2, 6);
        Range expandedRange = Range.expand(inputRange, 2, 2);
    ...
    public void testExpandWithZeroMargins() {
        Range inputRange = new Range(2, 6);
        Range expandedRange = Range.expand(inputRange, 0, 0);
    ...
    // Test for expand method with valid range and negative margins
    @Test
    public void testExpandValidRangeNegativeMargins() {
        Range range = new Range(1.0, 5.0);
        double lowerMargin = -0.1;
        double upperMargin = -0.2;
        ...
        // Check if the lower bound of the expanded range is correct
        assertEquals("The lower bound of the expanded range should be 1.4",
                1.4, expandedRange.getLowerBound(), 0.000000001d);

        // Check if the upper bound of the expanded range is correct
        assertEquals("The upper bound of the expanded range should be 4.6",
                4.6, expandedRange.getUpperBound(), 0.000000001d);
    }
  ...
  //-------------------------------- Scale -----------------------------------------
	 ...
	 @Test
	 public void shouldNotIntersectRanges() {
	     exampleRange = new Range(-1, 1);
	     Assume.assumeTrue(!expectedResult);
	...
	//--------------------------- Equals ---------------------------------
	 ...
	 public void YourExistingTestFile(double lowerBound, double upperBound, boolean expectedResult1) {
	 ...
	 @Test
	 public void testExpandToInclude_NullRangeWithNaNValue() {
	     Range result = Range.expandToInclude(null, Double.NaN);
	     assertEquals("Expanding null range with NaN value", new Range(Double.NaN, Double.NaN), result);
	 }
	 ...
	 @Test
	 public void testExpandToInclude_ExistingRangeWithNaNLowerBoundAndValidUpperBound() {
	     Range range = new Range(Double.NaN, 5.0);
	     Range result = Range.expandToInclude(range, 8.0);
	     assertEquals("Expanding existing range with NaN lower bound and valid upper bound", new Range(Double.NaN, 8.0), result);
	 }
